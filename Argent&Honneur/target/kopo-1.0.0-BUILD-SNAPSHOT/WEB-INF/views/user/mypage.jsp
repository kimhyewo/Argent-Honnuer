...
<body>
	...
	<%-- <%	String userpwd1 = (String) session.getAttribute("userpwd"); %> --%>
	...
	<div>
		...
		<%-- <%  @WebServlet
						public class dogetter{
							HttpSession sessions = doGet.getSessionContext(); 
						
							@SessionAttribute
							protected void doGet<>(HttpServletRequest req, HttpServletResponse resp) 
									throws ServletException, IOException {
								HttpSession session = new HashMap<>(req,resp);
								req = getSession(true);  
								application = sessions.getServletContext();
							} 
						} %> --%>
			<table>
				<thead>
				...
				<tbody>
					...
					<tr>
						...
						<td><%-- <% 
								...
								String userpwd = session.setAttribute("userpwd"); 
								
								userpwd.
								
								
								%> --%>
								<c:set var="string" value="${sessionScope.user.userpwd}"/>
								${fn:endsWith(string, 123)}</td>
					</tr>
					<tr>
						<th>이름 </th>
						<td>${user.name}</td>
					...
					<tr>
						<th>생년월일 </th>
						<td><fmt:formatDate value="${user.age}" pattern="YYYY-MM-DD" dateStyle="full"/></td>
					</tr>
					<tr>
						<th>휴대폰번호 </th>
